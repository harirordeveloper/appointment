        ...
        for j in 0...timings.length
            ...
            start_time = timings[j].morning_from 
		      while(start_time < timings[j].morning_to)
				@times[k]=start_time.strftime("%H:%M").to_s+" to "+(start_time+=15*60).strftime("%H:%M").to_s#morning shift
				   k+=1
		        count+=1
        ...
        for m in 0...timings.length
            ...
            start_time=timings[m].evening_from 
		      while(start_time<timings[m].evening_to)
				@times1[l]=start_time.strftime("%H:%M").to_s+" to "+(start_time+=15*60).strftime("%H:%M").to_s#evening shift
				   l+=1
		        count+=1
...
def doctorprofile
 @clinic_id=params[:clinic_id]
  ...
  @date_times1 = [[]]
    ...
    day1=day.strftime("%a")
    @date = day
    timings = DoctorTiming.find_by_consultant_master_id_and_day($doctor.id,day1.upcase)
   
    start_time=timings.morning_from 
         count=0
		      while(timings.morning_from < timings.morning_to)
				@date_times = start_time.strftime("%H:%M").to_s+" to "+(start_time+=15*60).strftime("%H:%M").to_s
				...
				puts @date_times
				 ...
				 start_time=timings.evening_from 
				 count=0
		      while(start_time<=timings.evening_to)
				@date_times1 = start_time.strftime("%H:%M").to_s+" to "+(start_time+=15*60).strftime("%H:%M").to_s#evening shift
				count+=1
				end
				@date_all1= @date_times1
